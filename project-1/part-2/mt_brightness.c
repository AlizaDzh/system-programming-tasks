...
static void brightness_processing_task(void *task_data,
  ...
  for (; position < end; position += step) {
    ...
    pixels[position] = (uint8_t)UTILS_CLAMP(
        pixels[position] * contrast + brightness, 0.0f, 255.0f);
    pixels[position + 1] = (uint8_t)UTILS_CLAMP(
        pixels[position + 1] * contrast + brightness, 0.0f, 255.0f);
    pixels[position + 2] = (uint8_t)UTILS_CLAMP(
        pixels[position + 2] * contrast + brightness, 0.0f, 255.0f);

#elif defined SIMD_INTRINSICS_IMPLEMENTATION

    __m128 b = _mm_set_ss(brightness);
    __m128 c = _mm_set_ss(contrast);

    __m512 bVec = _mm512_broadcastss_ps(b);
    __m512 cVec = _mm512_broadcastss_ps(c);
    __m512i ints =
        _mm512_cvtepu8_epi32(_mm_load_si128((__m128i *)&pixels[position]));
    __m512 floats = _mm512_cvtepi32_ps(ints);
    floats = _mm512_fmadd_ps(floats, cVec, bVec);
    ...
    __asm__ __volatile__("vbroadcastss (%0), %%zmm2\n\t"
                         "vbroadcastss (%1), %%zmm1\n\t"
                         "vpmovzxbd (%2,%3), %%zmm0\n\t"
                         "vcvtdq2ps %%zmm0, %%zmm0\n\t"
                         ...
                         "vcvtps2dq %%zmm0, %%zmm0\n\t"
                         "vpmovusdb %%zmm0, (%2,%3)\n\t" ::"S"(&brightness),
                         "D"(&contrast), "b"(pixels), "c"(position)
                         : "%zmm0", "%zmm1", "%zmm2");
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    fprintf(stderr,
            "Usage: %s <brightness> <contrast> <source file> <dest. file>\n",
            argv[0]);
    return result;
  }
  ...
  /* Main Image Processing Loop */
  {
    ...
    for (size_t position = 0; position < channels_count;
      ...
      threadpool_enqueue_task(threadpool, brightness_processing_task, task_data,
                              NULL);
    }
