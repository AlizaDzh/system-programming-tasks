...
#if !defined C_IMPLEMENTATION && !defined SIMD_INTRINSICS_IMPLEMENTATION &&    \
...
static void sepia_processing_task(void *task_data,
  ...
  for (; position < end; position += step) {
    ...
    uint32_t blue = pixels[position];
    uint32_t green = pixels[position + 1];
    uint32_t red = pixels[position + 2];

    pixels[position] = (uint8_t)UTILS_MIN(Sepia_Coefficients[0] * blue +
                                              Sepia_Coefficients[1] * green +
                                              Sepia_Coefficients[2] * red,
                                          255.0f);
    pixels[position + 1] = (uint8_t)UTILS_MIN(
        Sepia_Coefficients[3] * blue + Sepia_Coefficients[4] * green +
            Sepia_Coefficients[5] * red,
        255.0f);
    pixels[position + 2] = (uint8_t)UTILS_MIN(
        Sepia_Coefficients[6] * blue + Sepia_Coefficients[7] * green +
    ...
    static const float Sepia_Coefficients[] __attribute__((aligned(0x40))) = {
        0.272f, 0.349f, 0.393f, 1.0f, 0.272f, 0.349f, 0.393f, 1.0f,
        0.272f, 0.349f, 0.393f, 1.0f, 0.272f, 0.349f, 0.393f, 1.0f,
        0.534f, 0.686f, 0.769f, 1.0f, 0.534f, 0.686f, 0.769f, 1.0f,
        0.534f, 0.686f, 0.769f, 1.0f, 0.534f, 0.686f, 0.769f, 1.0f,
        0.131f, 0.168f, 0.189f, 1.0f, 0.131f, 0.168f, 0.189f, 1.0f,
        0.131f, 0.168f, 0.189f, 1.0f, 0.131f, 0.168f, 0.189f, 1.0f};
    ...
    __m512i ints =
        _mm512_cvtepu8_epi32(_mm_load_si128((__m128i *)&pixels[position]));
    ...
    __asm__ __volatile__(
        ...
        "vmovaps %%zmm0, %%zmm5\n\t"                // floats = temp2
        "vmovaps %%zmm0, %%zmm6\n\t"                // floats = temp3
        "vpermilps $0b11000000, %%zmm4, %%zmm4\n\t" // permute
        "vpermilps $0b11010101, %%zmm5, %%zmm5\n\t"
        "vpermilps $0b11101010, %%zmm6, %%zmm6\n\t"
        "vmulps %%zmm4, %%zmm1, %%zmm0\n\t"      // mfloats = coeff1 * temp1
        "vfmadd231ps %%zmm2, %%zmm5, %%zmm0\n\t" // floats = coeff2 * temp2 +
                                                 // floats
        "vfmadd231ps %%zmm3, %%zmm6, %%zmm0\n\t" // floats = coeff3 * temp3 +
                                                 // floats
        "vcvtps2dq %%zmm0, %%zmm0\n\t"           // floats ->ints
        "vpmovusdb %%zmm0, (%3, %4)\n\t"         // 32 -> 8 pix[pos] =  ...

        ::"r"(&Sepia_Coefficients[0]),
        "r"(&Sepia_Coefficients[16]), "r"(&Sepia_Coefficients[32]), "r"(pixels),
        "r"(position)
  ...
  ssize_t channels_left =
      __sync_sub_and_fetch(data->channels_left, (ssize_t)channels_to_process);
  if (channels_left <= 0) {
...
int main(int argc, char *argv[]) {
  ...
  /* Main Image Processing Loop */
  {
    ...
    for (size_t position = 0; position < channels_count;
         position += channels_per_thread) {
      ...
      threadpool_enqueue_task(threadpool, sepia_processing_task, task_data,
                              NULL);
    }
