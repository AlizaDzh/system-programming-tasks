...
int main(int argc, char *argv[])
{
    ...
    /* Main Image Processing Loop */
    {
        ...
        for (size_t position = 0; position < channels_count; position += step)
        {
            ...
            pixels[position] =
                (uint8_t)UTILS_MIN(
                    Sepia_Coefficients[0] * blue + Sepia_Coefficients[1] * green + Sepia_Coefficients[2] * red, 255.0f);

            pixels[position + 1] =
                (uint8_t)UTILS_MIN(
                    Sepia_Coefficients[3] * blue + Sepia_Coefficients[4] * green + Sepia_Coefficients[5] * red, 255.0f);

            pixels[position + 2] =
                (uint8_t)UTILS_MIN(
                    Sepia_Coefficients[6] * blue + Sepia_Coefficients[7] * green + Sepia_Coefficients[8] * red, 255.0f);
            ...
            static const float Sepia_Coefficients[] = {
                ...
                "vfmadd231ps %%zmm2, %%zmm5, %%zmm0\n\t"    // floats = coeff2 * temp2 + floats 
                "vfmadd231ps %%zmm3, %%zmm6, %%zmm0\n\t"    // floats = coeff3 * temp3 + floats 
                "vcvtps2dq %%zmm0, %%zmm0\n\t"      // floats ->ints
                "vpmovusdb %%zmm0, (%3, %4)\n\t"    // 32 -> 8 pix[pos] =  ...                         
                
                ::
                    "r"(&Sepia_Coefficients[0]), "r"(&Sepia_Coefficients[16]), "r"(&Sepia_Coefficients[32]), "r"(pixels), "r"(position)
